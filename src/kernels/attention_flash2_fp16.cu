...
__global__ void attention_flash_2_fp16_kernel(
        ...
        float scale)
{
    ...
    constexpr size_t Q_BYTES = size_t(2) * Br * D;                         // Br*D halves
    constexpr size_t SP_BYTES = size_t(4) * Br * Bc + size_t(2) * Br * Bc; // S floats + P halves
    constexpr size_t KV_OFF = (Q_BYTES > SP_BYTES) ? Q_BYTES : SP_BYTES;

    half* Q_i = reinterpret_cast<half*>(smem);                                    // [Br][D]
    float* S_ij = reinterpret_cast<float*>(smem);                                 // [Br][Bc]
    half* P_ij = reinterpret_cast<half*>(S_ij + Br * Bc);                         // [Br][Bc]
    half* K_j = reinterpret_cast<half*>(reinterpret_cast<char*>(smem) + KV_OFF);  // [Bc][D]
    half* V_j = K_j + Bc * D;                                                     // [Bc][D]
...
#pragma unroll
    ...
    for (int j = 0; j < Tc; ++j) {
        for (int i = tid; i < Bc * D; i += total_threads) {
            ...
            K_j[i] = __float2half(K[kv_idx]);
            V_j[i] = __float2half(V[kv_idx]);
        }
        __syncthreads();
...
#pragma unroll
        for (int k = 0; k < PV_STEPS; ++k) {
            wmma::load_matrix_sync(p_frag[k], P_ij + warp_id * WMMA_M * Bc + k * WMMA_K, Bc);
        }
                   ...
                   float* O, int N, float scale, int B)
{
    ...
    constexpr size_t SP_BYTES = size_t(4) * Br * Bc + size_t(2) * Br * Bc;
    constexpr size_t KV_OFF = (Q_BYTES > SP_BYTES) ? Q_BYTES : SP_BYTES;
    constexpr size_t smem_bytes = KV_OFF + size_t(2) * (size_t(2) * Bc * D);
    ...
    static const bool configured = [] {
        ...
        if ((int)smem_bytes > max_smem) {
            fprintf(stderr, "fa2: requested %zu B smem > device max %d B (D=%d Br=%d Bc=%d)\n",
                    smem_bytes, max_smem, D, Br, Bc);
            exit(1);
        }
        cudaError_t e = cudaFuncSetAttribute(
                attention_flash_2_fp16_kernel<D, Br, Bc>,
    ...
    int Tr = (N + Br - 1) / Br;
    attention_flash_2_fp16_kernel<D, Br, Bc> <<<dim3(Tr, B), THREADS, smem_bytes>>>(Q, K, V, O, N, scale);
